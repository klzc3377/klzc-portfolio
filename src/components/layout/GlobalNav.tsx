  ...
  return (
    <>
      <header className={`floating-nav ${path === '/' ? 'is-tunnel-nav' : ''}`}>
        <RouteLink className="brand" href="/" navigate={navigate} ariaLabel="返回首页 / KLzc home">
          <BrandMark variant="compact" title={null} aria-hidden="true" />
          <span>Cheng Zhang</span>
        </RouteLink>
        <nav className="desktop-links" aria-label="主导航 / Main navigation">
          {navItems.map((item) => (
          ...
          <RouteLink href="/#contact" navigate={navigate}><span>联系</span><small>Contact</small></RouteLink>
        </nav>
        <button
          ref={triggerRef}
          className="mobile-menu-trigger"
          type="button"
          aria-label={menuOpen ? '关闭导航 / Close navigation' : '打开导航 / Open navigation'}
        ...
        {menuOpen ? (
          ...
          >
            <button
              ...
              type="button"
              aria-label="关闭导航 / Close navigation"
